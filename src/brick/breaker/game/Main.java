...
class GamePlay extends JPanel implements KeyListener, ActionListener  {
	...
	private int totalBricks = 21;
        ...
        private int lifeChances = 3; // Number of life chances
        private Image lifeIcon; // Life icon image
	...
	public GamePlay() {
		...
		timer.start();
                ...
                // Load the life icon image and resize it
                ImageIcon img = new ImageIcon(getClass().getResource("/Icons/live_icon.png"));
                lifeIcon = img.getImage();
	...
	public void paint(Graphics g) {
		...
		g.fillOval(ballposX, ballposY, 20, 20);
                ...
                // Draw life icons based on lifeChances
                for (int i = 0; i < lifeChances; i++) {
                    g.drawImage(lifeIcon, 20 + i * 40, 10, this);
                }
		...
		if (totalBricks <= 0) { // if all bricks are destroyed then you win
			...
			g.drawString("Press Enter to Restart.", 230, 350);
                ...
                if (ballposY > 570) {
                    // Decrement life chances
                    lifeChances--;

                    // Check if no life chances left
                    if (lifeChances <= 0) {
            ...
            if (lifeChances > 0) {
                ...
                g.setFont(new Font("MV Boli", Font.BOLD, 20));
                g.drawString("No lives left! Press Enter to Restart", 180, 350);
            }
                        gameOver();
                    } else {
        ...
        for (int i = 0; i < map.map.length; i++) { // Ball - Brick interaction
            for (int j = 0; j < map.map[0].length; j++) {  // map.map[0].length is the number of columns
                if (map.map[i][j] > 0) {
                    ...
                    if (totalBricks <= 0 || lifeChances <= 0) {
                        gameOver();
                    }
...
class SoundPlayer {
    public static void playBrickSound() {
        playSound("/Sounds/brick_sound.wav"); // Adjust file extension if necessary
    }

    public static void playCollisionSound() {
        playSound("/Sounds/collision_sound.wav"); // Adjust file extension if necessary
    }

     private static void playSound(String soundFilePath) {
        try {
            Clip clip = AudioSystem.getClip();
            AudioInputStream inputStream = AudioSystem.getAudioInputStream(SoundPlayer.class.getResourceAsStream(soundFilePath));
            clip.open(inputStream);
            clip.start();
        } catch (LineUnavailableException e) {
            System.err.println("LineUnavailableException: " + e.getMessage());
        } catch (UnsupportedAudioFileException e) {
            System.err.println("UnsupportedAudioFileException: " + e.getMessage());
        } catch (IOException e) {
